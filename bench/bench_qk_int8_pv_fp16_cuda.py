...
if args.pv_accum_dtype == "fp32":
    kernel = (
        qattn.qk_int8_sv_f16_accum_f32_attn
    )  # the kernel with fully fp32 accumulator
elif args.pv_accum_dtype == "fp16+fp32":
    kernel = (
        qattn.qk_int8_sv_f16_accum_f16_attn_inst_buf
    )  # the kernel with fp32 longterm buffer and fp16 shortterm accumulator
elif args.pv_accum_dtype == "fp16":
    kernel = (
        qattn.qk_int8_sv_f16_accum_f16_attn
    )  # the kernel with fully fp16 accumulator
...
for seq_len in {1024, 2048, 4096, 8192, 16384, 32768}:
    ...
    q = torch.randint(
        -95, 95, (batch, seq_len, head, headdim), dtype=torch.int8, device="cuda"
    )
    k = torch.randint(
        -95, 95, (batch, seq_len, head, headdim), dtype=torch.int8, device="cuda"
    )
    ...
    if args.quant_gran == "per_warp":
        q_scale = torch.randn(
            batch, head, seq_len // WARP_Q, dtype=torch.float, device="cuda"
        )
        k_scale = torch.randn(
            batch, head, seq_len // WARP_K, dtype=torch.float, device="cuda"
        )
    elif args.quant_gran == "per_thread":
        q_scale = torch.randn(
            batch, head, seq_len // WARP_Q * 8, dtype=torch.float, device="cuda"
        )
        k_scale = torch.randn(
            batch, head, seq_len // WARP_K * 4, dtype=torch.float, device="cuda"
        )
