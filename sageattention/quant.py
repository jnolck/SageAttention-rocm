...
def per_block_int8(
    ...
    sm80_compile.quant_per_block_int8_cuda(
        q, q_int8, q_scale, sm_scale, BLKQ, _tensor_layout
    )
    if km is not None:
        km = km.squeeze(1) if _tensor_layout == 0 else km.squeeze(2)
        sm80_compile.quant_per_block_int8_fuse_sub_mean_cuda(
            k, km, k_int8, k_scale, BLKK, _tensor_layout
        )
    else:
        sm80_compile.quant_per_block_int8_cuda(k, k_int8, k_scale, BLKK, _tensor_layout)
...
def per_warp_int8(
    ...
    sm80_compile.quant_per_warp_int8_cuda(
...
def sub_mean(v: torch.Tensor, tensor_layout: str = "HND"):
    ...
    sm80_compile.sub_mean_cuda(v, vm, v_smoothed, _tensor_layout)
...
#         )
#
#     sage_fused_quant.transpose_pad_permute_cuda(v, v_transposed_permutted, _tensor_layout)
#
#     v_fp8 = torch.empty(
...
#     else:
#         sm80_compile.scale_fuse_quant_cuda(
