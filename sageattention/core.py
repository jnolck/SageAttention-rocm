...
def get_cuda_arch_versions():
    cuda_archs = []
    gfx = torch.cuda.get_device_properties(0)
    cuda_archs.append(f"{gfx.gcnArchName}")
    return cuda_archs
...
def sageattn(
    ...
    **kwargs: Any,
    ...
    # use hsa override to test triton path
    elif arch == "gfx1101":
...
def sageattn_qk_int8_pv_fp16_cuda(
    ...
    **kwargs: Any,
    ...
    if pv_accum_dtype == "fp32":
        ...
        #     _return_lse,
        # )
        print("Warning: inst_buf kernel missing. Falling back to fp32 accumulator.")
        v = v.to(torch.float16)
        lse = sm80_compile.qk_int8_sv_f16_accum_f32_attn(
            q_int8,
            k_int8,
